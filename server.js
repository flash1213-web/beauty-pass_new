...
try {
  ...
  console.error('? ?????? ???????? email transporter:', err);
}
...
async function send2FACode(user) {
  ...
  const expires = new Date(Date.now() + 5 * 60 * 1000); // 5 ?????
  user.twoFACode = code;
  user.twoFACodeExpires = expires;
  await user.save();

  console.log(`?? ???????? 2FA ???? ?? ${user.email}...`);

  // ????????? ????????? email
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    console.error('? EMAIL_USER ??? EMAIL_PASS ?? ????????? ? .env!');
    console.log('');
    console.log('+------------------------------------------------------------+');
    ...
    html: `
      <div style="font-family: Arial, sans-serif; background-color: #f4f4f4; padding: 20px;">
        <div style="max-width: 600px; margin: auto; background: white; padding: 30px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
          ...
          <p style="font-size: 16px; color: #333;">?????????, ${user.firstName || user.login}!</p>
          ...
          <p style="font-size: 14px; color: #888;">???? ????????? 5 ????.</p>
          <p style="font-size: 14px; color: #888;">?? ????? ?? ????????? ????, ?????????? ???????? ?? ??????.</p>
          <hr style="margin: 20px 0; border: none; border-top: 1px solid #eee;">
          <p style="font-size: 12px; color: #aaa; text-align: center;">Beauty Pass - ?????? ????????? ???????? ??</p>
        </div>
      </div>
    `,
  };
  ...
  try {
    ...
    console.log(`? 2FA ???? ???????????: ${user.email} (messageId: ${info.messageId})`);
    return code;
  } catch (error) {
...
async function sendBookingQREmail(booking, user) {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; background-color: #f4f4f4; padding: 20px;">
        <div style="max-width: 600px; margin: auto; background: white; padding: 30px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
          ...
          <p style="font-size: 16px; color: #333;">?????????, ${user.firstName || '????????????'}!</p>
          <p style="font-size: 16px; color: #333;">?????? ??????? ??????????????:</p>
          
          <div style="background-color: #ffe0e8; padding: 20px; border-radius: 10px; margin: 20px 0;">
            <p><strong>?? ??????:</strong> ${booking.salonName}</p>
            ${booking.specialistName ? `<p><strong>????? ???????????:</strong> ${booking.specialistName}</p>` : ''}
            <p><strong>?? ?????????:</strong> ${booking.serviceName}</p>
            <p><strong>?? ??????:</strong> ${booking.date}</p>
            <p><strong>?? ???:</strong> ${booking.time}</p>
            <p><strong>?? ????:</strong> ${booking.bpPrice} BP</p>
            <p><strong>?? ????:</strong> <span style="font-size: 20px; font-weight: bold; color: #ff6b9d;">${booking.bookingCode}</span></p>
          </div>
          
          <div style="text-align: center; margin: 30px 0; padding: 20px; background: #fff;">
            <p style="font-size: 14px; color: #666; margin-bottom: 15px;">??????? ?? QR ???? ???????:</p>
            ...
            <p style="font-size: 14px; color: #666; margin-top: 15px;">?? ??????? ????:</p>
  ...
  try {
    ...
    console.log(`? QR email ???????????: ${user.email}`);
  } catch (error) {
...
app.get('/api/check-availability', async (req, res) => {
  try {
    ...
    if (!type || !value) return res.status(400).json({ message: '???????? ??????' });
...
app.get('/api/public-stats', async (req, res) => {
  try {
    ...
    console.log(`?? ??????????: ???????=${clientsCount}, ??????=${salonsCount}, ????????????=${bookingsCount}`);
...
app.get('/api/salons', async (req, res) => {
  try {
    ...
    console.log(`?? ??????? ???????: ${salonsForFrontend.length}`);
...
app.get('/api/salons/:name/slots', async (req, res) => {
  try {
    ...
    if (!date) return res.status(400).json({ message: '??????? ????' });
    
    const salon = await Salon.findOne({ name });
    if (!salon) return res.status(404).json({ message: '????? ?? ??????' });
...
app.get('/api/services', async (req, res) => {
  try {
    ...
    const servicesForFrontend = services.map(service => ({
      ...
      bp: service.bpPrice,
      salonName: service.ownerId?.salonName || '?????'
    }));
    console.log(`?? ??????? ?????: ${servicesForFrontend.length}`);
...
app.post('/api/specialists/:specialistId/review', authMiddleware, async (req, res) => {
  try {
    ...
    if (!rating || rating < 1 || rating > 5) {
      ...
      return res.status(400).json({ message: '???????? ???? ???? 1-??? 5-??? / Rating must be 1-5' });
    }
    ...
    if (existingReviewIndex >= 0) {
      ...
      if (user) {
        ...
        if (newLevel > oldLevel && newLevel % 5 === 0) {
          const levelBonus = newLevel * 10; // 50 BP ?? 5 ???????, 100 BP ?? 10 ? ?.?.
...
app.post('/api/salon/specialists/:specialistId/slots', authMiddleware, async (req, res) => {
  try {
    ...
    if (salon.userType !== 'salon') {
      return res.status(403).json({ message: '?????? ???????? ???????? / Only salons can create slots' });
    }
    ...
    if (specialist.salonId.toString() !== salon._id.toString()) {
      return res.status(403).json({ message: '?? ??????????? ?? ???????? ?????? ?????? / This specialist does not belong to your salon' });
    }
    ...
    res.status(201).json({ 
      message: `??????? ${createdSlots.length} ????? / Created ${createdSlots.length} slots`,
      slots: createdSlots 
    });
...
app.post('/api/booking/slot', authMiddleware, async (req, res) => {
  try {
    ...
    if (user.userType === 'salon') {
      return res.status(403).json({ message: '???????? ?? ???????? ??????? ????????. ?????? ?????????????? / Salons cannot make bookings. Clients only' });
    }
    
    if (!slotId) {
      return res.status(400).json({ message: '??????? ????? / Select a slot' });
    }
    ...
    if ((user.beautyPoints || 0) < price) {
      return res.status(400).json({ message: `???????????? BP. ??????? ${price} BP / Insufficient BP. Need ${price} BP` });
    }
    ...
    let levelBonus = 0;
    if (newLevel > oldLevel && newLevel % 5 === 0) {
      levelBonus = newLevel * 10; // 50 BP ?? 5 ???????, 100 BP ?? 10 ? ?.?.
      user.beautyPoints = (user.beautyPoints || 0) + levelBonus;
      console.log(`?? Level ${newLevel} bonus: +${levelBonus} BP for ${user.email}`);
    }
...
app.post('/api/booking/specialist', authMiddleware, async (req, res) => {
  try {
    ...
    console.log(`? ???????????? ???????: ${booking.bookingCode} | ${user.email} ? ${salonName} (${specialistName})`);
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    if (password.length < 8) {
      return res.status(400).json({ message: '?????? ???? ???? ??????? 8 ???????' });
    }
    ...
    if (birthDate) {
      ...
      if (age < 13) {
        return res.status(400).json({ message: '?????????? ?????: 13 ????' });
      }
      if (age > 120) {
        return res.status(400).json({ message: '???????? ????????? ??????' });
      }
    ...
    console.log('?? ??????? ????????????:', { email: user.email, passwordLength: password.length });
    await user.save();
    console.log('? ???????????? ????????, ?????? ?????????:', user.password ? '??' : '???');
    
    // ????????? ????? ????????
    if (referrer) {
      const REFERRAL_BONUS_BP = 10; // 10 BP ?? ????????
      const REFERRAL_BONUS_XP = 20; // 20 XP ?? ????????
      ...
      await referrer.save();
      console.log(`? ????????? ${REFERRAL_BONUS_BP} BP ? ${REFERRAL_BONUS_XP} XP ???????????? ${referrer.email} ?? ????????`);
    }

    try {
      await send2FACode(user);
      res.status(201).json({ message: '??????????? ????????????! ????????? ??????? ?? ????????? ????.', needVerification: true, email: user.email });
    } catch (emailError) {
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    console.log('?? ????????? ??????, ??? ? ????:', user.password ? user.password.substring(0, 20) + '...' : '???????????');
          ...
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
              ...
              <div style="background: #f8f9fa; padding: 30px; border-radius: 10px;">
                ...
                <p style="color: #999; font-size: 14px;">???? ????????? 15 ????? ????????????.</p>
          ...
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
              ...
              <div style="background: #d4edda; padding: 30px; border-radius: 10px; border: 1px solid #c3e6cb;">
                <h2 style="color: #155724; margin-bottom: 20px;">? ?????? ?????????? ????????</h2>
                <p style="color: #155724;">?????? ????????? ?????? ?????????? ????????.</p>
                <p style="color: #856404; margin-top: 20px;">?? ????? ?? ???????? ??????, ???????????? ??????????????.</p>
              </div>
              <div style="text-align: center; margin-top: 30px; color: #999; font-size: 12px;">
                � 2026 Beauty Pass. ????? ?????? ???????.
              </div>
...
app.post('/api/auth/send-sms-code', async (req, res) => {
  try {
    ...
    res.json({ 
      message: `SMS ??? ???????????: ${maskedPhone}`,
      method: 'sms'
    });
  } catch (error) {
    console.error('SMS 2FA error:', error);
    res.status(500).json({ message: '??????? SMS ???????????' });
  }
});
...
app.post('/api/auth/select-2fa-method', async (req, res) => {
  try {
    ...
    if (method === 'sms') {
      ...
      await sendSMS2FACode(user, user.phone);
      const maskedPhone = user.phone.replace(/(\d{3})\d{4}(\d{2})/, '$1****$2');
      res.json({ message: `???? ??????????? SMS-??: ${maskedPhone}`, method: 'sms' });
    } else {
      await send2FACode(user);
      const maskedEmail = user.email.replace(/(.{2})(.*)(@.*)/, '$1***$3');
      res.json({ message: `???? ??????????? ?????????: ${maskedEmail}`, method: 'email' });
    }
  } catch (error) {
...
app.put('/api/admin/users/:id/toggle-ban', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ message: user.isBanned ? '???????????? ????????????' : '???????????? ?????????????', user });
  } catch (error) {
    res.status(500).json({ message: '?????? ???????' });
  }
});
  ...
  try {
    ...
    const analytics = await Promise.all(salons.map(async (salon) => {
      ...
      return {
        ...
        // ???????
        totalRevenue: salon.salonRevenue || 0,           // ????? ?????????? BP
        pendingRevenue: salon.salonPendingRevenue || 0,  // ? escrow
...
app.get('/api/admin/salon-analytics/:salonId', authMiddleware, async (req, res) => {
  try {
    ...
    // ??????? ?????? (???? ?????)
    const withdrawalHistory = []; // TODO: ????? ???????? ?????? Withdrawal
  ...
  try {
    ...
    if (amount > availableForWithdrawal) {
      return res.status(400).json({ 
        message: `???????????? ???????. ????????: ${availableForWithdrawal} BP` 
      });
    }
...
app.get('/api/admin/export-analytics', authMiddleware, async (req, res) => {
  try {
    ...
    const overviewData = [
      ...
      ['?????????????', specialists.length],
      [],
      ['??????? ?????????? (BP)', salons.reduce((sum, s) => sum + (s.salonRevenue || 0), 0)],
      ['Escrow-?? (BP)', salons.reduce((sum, s) => sum + (s.salonPendingRevenue || 0), 0)],
      ['????????? (BP)', salons.reduce((sum, s) => sum + (s.salonWithdrawnRevenue || 0), 0)]
    ];
    const wsOverview = XLSX.utils.aoa_to_sheet(overviewData);
    XLSX.utils.book_append_sheet(wb, wsOverview, '?????????');
    
    // === ???? 2: ?????? ===
    const salonHeaders = ['??????? ??????', 'Email', '????????', '?????????', '???????????', '?????????? (BP)', 'Escrow (BP)', '????????? (BP)', '????????', '???????????', '??????????'];
    ...
    XLSX.utils.book_append_sheet(wb, wsSalons, '????????');
    
    // === ???? 3: ???????????? ===
    const bookingHeaders = ['????', '???????', 'Email', '??????', '???????', '?????????', '??????', '???', 'BP', '???????', '???????', '??????? ??????'];
    ...
    XLSX.utils.book_append_sheet(wb, wsBookings, '????????');
    
    // === ???? 4: ???????????? ===
    const userHeaders = ['??????', '?????', 'Email', '????????', 'BP ???????', '????????', '???????????'];
    ...
    XLSX.utils.book_append_sheet(wb, wsUsers, '?????????????');
    ...
    XLSX.utils.book_append_sheet(wb, wsSpecs, '?????????????');
    ...
    XLSX.utils.book_append_sheet(wb, wsCats, '???????????');
...
app.get('/api/admin/export-salon/:salonId', authMiddleware, async (req, res) => {
  try {
    ...
    const salonInfo = [
      ...
      ['????????:', salon.salonPhone || '-'],
      ['?????????:', salon.salonAddress || '-'],
      ['???????????:', salon.createdAt ? new Date(salon.createdAt).toLocaleDateString('ka-GE') : '-'],
      [],
      ['=== ????????? ==='],
      ['??????? ??????????:', `${salon.salonRevenue || 0} BP`],
      ['Escrow-??:', `${salon.salonPendingRevenue || 0} BP`],
      ['?????????:', `${salon.salonWithdrawnRevenue || 0} BP`],
      ['????????:', `${(salon.salonRevenue || 0) - (salon.salonWithdrawnRevenue || 0)} BP`],
      [],
      ['=== ?????????? ==='],
      ['??? ????????:', salon.salonTotalBookings || 0],
      ['???????????:', salon.salonCompletedBookings || 0],
      ['??????????:', salon.salonCancelledBookings || 0]
    ];
    const wsInfo = XLSX.utils.aoa_to_sheet(salonInfo);
    XLSX.utils.book_append_sheet(wb, wsInfo, '??????????');
    
    // === ???? 2: ???????????? ===
    const bookingHeaders = ['????', '???????', '????????', '???????', '???????????', '??????', '???', 'BP', '???????', '???????'];
    const bookingData = bookings.map(b => [
      b.bookingCode,
      b.userId ? `${b.userId.firstName || ''} ${b.userId.lastName || ''}` : b.clientName || '-',
...
app.post('/api/packages/buy', authMiddleware, async (req, res) => {
  try {
    ...
    if (!pkg) return res.status(404).json({ message: '????? ?? ??????' });
    
    if (user.balance < price) {
      return res.status(400).json({ message: '???????????? ??????? ?? ???????. ???????????? ???????.' });
    }
    
    // ????????? BP ?? ?????? ?????? (tokens ? Package = BP)
    ...
    // ????????????? ???????? ????
    const now = new Date();
    const expiresAt = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000); // 30 ????
    ...
    // Отправляем email с чеком о покупке
    const lastPurchase = user.purchases[user.purchases.length - 1];
    sendPurchaseReceiptEmail(user, lastPurchase, pkg).catch(err => console.error('Email receipt error:', err));
    
    console.log(`? User ${user.email} purchased ${pkg.plan}: -${price}?, +${bpToAdd} BP`);
    
    res.json({ 
      message: `${pkg.plan} ?????????! +${bpToAdd} BP ???????????`,
      user: {
        balance: user.balance,
        beautyPoints: user.beautyPoints,
...
app.post('/api/bookings', authMiddleware, async (req, res) => {
  try {
    ...
    if (!service) return res.status(404).json({ message: '?????? ?? ???????' });
    
    const priceBP = service.bpPrice;
    if (user.balance < priceBP) {
      return res.status(400).json({ message: '???????????? BP. ????????? ?????? ? ???????.' });
    }
    
    user.balance -= priceBP;
    // ????????? gamification ??????????
    user.totalBookings = (user.totalBookings || 0) + 1;
    user.xp = (user.xp || 0) + 10; // 10 XP ?? ????????????
...
app.delete('/api/salon-owner/services/:id', authMiddleware, salonOwnerMiddleware, async (req, res) => { 
  try { 
    ...
    if (result.deletedCount === 0) return res.status(404).json({ message: '?????? ?? ???????' }); 
    await updateSalonServices(req.user._id); 
    res.json({ message: '?????? ???????' }); 
  } catch (error) { 
    res.status(500).json({ message: '?????? ???????' }); 
...
app.post('/api/slots', authMiddleware, salonOwnerMiddleware, async (req, res) => {
  try {
    ...
    await slot.save();
    res.status(201).json({ message: '????? ??????? ?????????!', slot });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: '?????? ???????' });
  }
...
app.get('/api/salon-owner/bookings', authMiddleware, salonOwnerMiddleware, async (req, res) => {
  try { 
    ...
    const bookingsWithClientInfo = bookings.map(booking => ({
      ...booking.toObject(),
      clientName: booking.userId ? `${booking.userId.firstName} ${booking.userId.lastName}` : '??????',
      clientPhone: booking.userId ? booking.userId.phone : '??? ????????',
    }));
...
app.get('/api/salon-owner/finance', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      finance: {
        totalRevenue: salon.salonRevenue || 0,           // ????? ?????????? (released)
        pendingRevenue: salon.salonPendingRevenue || 0,  // ? escrow (??????? ?????????????)
        withdrawnRevenue: salon.salonWithdrawnRevenue || 0, // ??? ?????????
        availableForWithdrawal                            // ???????? ? ??????
      },
...
app.post('/api/transactions/create', authMiddleware, async (req, res) => {
  try {
    ...
    if (!accessToken) { 
      throw new Error('?? ??????? ???????? access token ?? TBC'); 
    }
...
app.get('/api/transactions/:transactionId/status', async (req, res) => {
  try {
    ...
    if (!transaction) return res.status(404).json({ message: '?????????? ?? ???????' });
...
app.get('/api/available-slots', async (req, res) => {
  try {
    ...
    console.log(`?? ????????? ?????: ??????? ${enrichedSlots.length}`);
...
app.post('/api/salon-owner/add-slot', authMiddleware, async (req, res) => {
  try {
    ...
    console.log(`? ???? ????????: ${req.user.salonName} - ${serviceName} ${date} ${time}`);
    res.status(201).json({ message: '????? ???????????', slot });
  } catch (error) {
...
app.post('/api/salon-owner/add-slots-batch', authMiddleware, async (req, res) => {
  try {
    ...
    console.log(`? ????????? ${slots.length} ?????? ??? ${req.user.salonName}`);
    res.status(201).json({ message: `${slots.length} ????? ???????????`, slots });
  } catch (error) {
...
app.post('/api/salon-owner/specialists', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(201).json({ message: '??????????? ???????????', specialist });
  } catch (error) {
...
app.delete('/api/salon-owner/gallery/:index', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ message: '????? ????????', gallery: salon.gallery });
  } catch (error) {
...
app.post('/api/booking/create', authMiddleware, async (req, res) => {
  try {
    ...
    if ((user.beautyPoints || 0) < bpPrice) {
      return res.status(400).json({ 
        message: `???????????? BP. ???????: ${bpPrice} BP, ????? ?????: ${user.beautyPoints || 0} BP` 
      });
    }
    ...
    console.log(`? ???????????? ???????: ${bookingCode} ??? ${user.email} (${bpPrice} BP ? ??????)`);
...
app.put('/api/booking/:bookingId/cancel', authMiddleware, async (req, res) => {
  try {
    ...
    if (daysUntil >= 2) {
      ...
      console.log(`? ???????????? ???????? (??????? ???????): ${booking.bookingCode}, ${bpPrice} BP`);
      ...
      console.log(`? ???????????? ???????? (BP ??????): ${booking.bookingCode}, ${bpPrice} BP -> ${salon?.salonName}`);
      
      res.json({ 
        message: `??????? ???????????. ${bpPrice} BP ???????? ?????? (???????? 2 ????? ???????? ????).`,
        refunded: false,
...
app.put('/api/booking/:bookingId/change', authMiddleware, async (req, res) => {
  try {
    ...
    console.log(`? ???????????? ????????: ${booking.bookingCode}, ${oldDate} ${oldTime} -> ${newSlot.date} ${newSlot.time}`);
    
    res.json({ 
      message: `??????? ????????: ${newSlot.date} ${newSlot.time}`,
      booking: {
        ...booking.toObject(),
        date: newSlot.date,
        time: newSlot.time
...
app.post('/api/salon-owner/confirm-booking', authMiddleware, async (req, res) => {
  try {
    ...
    const thirtyMinutes = 30 * 60 * 1000; // 30 ????? ? ?????????????
...
app.post('/api/messages/send', authMiddleware, async (req, res) => {
  try {
    ...
    if (message.length > 1000) {
      return res.status(400).json({ message: '??????????? ?????? ??????? (????. 1000 ???????)' });
    }
  ...
  try {
    ...
    res.json({
      ...
      reviews: (salon.reviews || []).map(r => ({
        userName: r.userName || (r.userId ? `${r.userId.firstName} ${r.userId.lastName}` : '???????'),
        rating: r.rating,
        comment: r.comment,
...
app.post('/api/map/salon/:id/review', authMiddleware, async (req, res) => {
  try {
    ...
    if (!rating || rating < 1 || rating > 5) {
      return res.status(400).json({ message: '???????? ???? ???? 1-??? 5-???' });
    }
...
app.get('/api/map/categories', async (req, res) => {
  try {
    const categories = [
      { id: 'nails', name: '?????????', nameEn: 'Nails', icon: '??' },
      { id: 'hair', name: '???', nameEn: 'Hair', icon: '??' },
      { id: 'face', name: '????', nameEn: 'Face', icon: '??' },
      { id: 'body', name: '??????', nameEn: 'Body', icon: '??' },
      { id: 'makeup', name: '???????', nameEn: 'Makeup', icon: '??' },
      { id: 'spa', name: '???', nameEn: 'Spa', icon: '??' }
    ];
    res.json(categories);
  } catch (error) {
    res.status(500).json({ message: '?????? ???????' });
